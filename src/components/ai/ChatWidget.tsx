...
const API_ENDPOINT = "/api/ai" as const;
...
/**
 * Generates a cryptographically secure unique identifier for messages.
 */
...
/**
 * Normalizes navigation routes by prepending dashboard prefix if applicable.
 */
...
/**
 * Lightweight, robust markdown parser sub-component.
 */
...
/**
 * Helper to process inline formatting rules (**bold**, `code`, *italic*).
 */
...
const LoadingIndicator = memo(() => (
  ...
  >
    ...
    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">
      Evaluating Financial Context Vector Stream...
    </span>
  </motion.div>
));
...
export default function ChatWidget() {
  ...
  // Hydrated initial greeting computed from current finance state
  const initialMessage = useMemo((): ChatMessage => {
    ...
    if (!finance || finance.transactionCount === 0) {
      return {
        ...
        role: "assistant",
        content:
          "Yo! I'm **VibeCheck AI**, your financial core copilot. 🌿 Your transaction index is looking fresh. Log some expenses or income, and let's optimize your wealth momentum.",
        timestamp: Date.now(),
      };
    }
    ...
    return {
      id: "initial_hydrated",
      role: "assistant",
      content: `Yo **${userName}**! VibeCheck AI is fully synced. You're holding **₹${totalSavings}** in net savings with a **${currentStreak}-day streak** active. What's our next financial strategy?`,
      timestamp: Date.now(),
    };
  }, [finance]);
  ...
  // Synchronize initial state if finance context refreshes asynchronously
  useEffect(() => {
    setMessages((prev) => {
      if (
        prev.length === 1 &&
  ...
  // Global keybinds & custom event listeners
  useEffect(() => {
  ...
  // Cleanup pending network streams on component unmount
  useEffect(() => {
    return () => {
  ...
  const executePromptStream = useCallback(
    async (textPrompt: string) => {
      ...
      try {
        ...
        if (!response.ok) {
          let errorMsg = `Server error status ${response.status}. Failed to communicate with VibeCheck AI core.`;
          try {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            aria-label="Open VibeCheck AI Assistant"
            ...
            className={cn(
              "fixed bottom-24 right-4 z-[100] flex h-14 w-14 items-center justify-center rounded-full border-2 border-zinc-950 bg-amber-400 text-zinc-950 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all sm:right-6 sm:h-16 sm:w-16 lg:bottom-8"
            )}
          >
            <Sparkles size={26} className="fill-zinc-950" />
          </motion.button>
        )}
      </AnimatePresence>

      {/* Main Interactive AI Interface Drawer */}
      <AnimatePresence>
        {isOpen && (
          <motion.aside
            role="dialog"
          ...
          >
            ...
            {/* User Input & Interactive Controller Footer */}
