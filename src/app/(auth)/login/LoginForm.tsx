  ...
  ({
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative flex items-center">
        {icon && (
          <div className="absolute left-4 text-[var(--text-light)]/40 pointer-events-none z-10" aria-hidden="true">
            {icon}
          </div>
        )}
        <input
...
export default function LoginForm() {
  ...
  const handleResendOtpRequest = async () => {
    const email = state.fields.email.trim().toLowerCase();
    if (!email || !EMAIL_REGEX.test(email)) {
      dispatch({ type: "SET_ERROR", payload: "Valid email address required to resend passcode." });
      return;
    }
    ...
    try {
      ...
      dispatch({
        type: "SET_SUCCESS",
        payload: "New verification passcode dispatched to registered email.",
      });
    } catch (err) {
